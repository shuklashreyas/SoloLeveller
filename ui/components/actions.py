...
class ActionsBar(tk.Frame):
    """
    Action bar with contrast-aware text colors.
    Uses PRIMARY_TEXT / ACCENT_TEXT / CARD_TEXT computed in constants.set_theme().
    """
    def __init__(
        self,
        master,
    ...
    on_items=None,
        ...
        self.sin_btn = RoundButton(
            left, "Sin",
            fill=COLORS["ACCENT"],
            hover_fill=COLORS.get("ACCENT_HOVER", COLORS["ACCENT"]),
    ...
    def set_contracts_badge(self, n: int):
        """Show 'Contracts (n)' and a small note when there are fresh offers."""
